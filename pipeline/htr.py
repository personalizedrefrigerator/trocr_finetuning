...
def mask_image_with_polygon(image: PIL.Image, polygon: list[tuple]):
...
def crop_image_to_polygon(image: PIL.Image, polygon: list[tuple]):
...
def segment_image(image: PIL.Image):
...
def recognise_text(image: PIL.Image):
	"""
	Attepts to recognise handwritten text from the provided [image].
	"""